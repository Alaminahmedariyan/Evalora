const fs = require('fs');
const path = 'D:\\Mission Complete\\assignments\\assignment-6-server\\postman.collection.json';
const collection = JSON.parse(fs.readFileSync(path, 'utf8'));

const authFolder = collection.item.find(f => f.name === '1. Auth');
const registerRecruiter = authFolder.item.find((r) => r.name === 'Register Recruiter');
...
const updateRoleRequest = {
  ...
  request: {
    ...
    body: { mode: 'raw', raw: JSON.stringify({ role: 'RECRUITER' }, null, 2), options: { raw: { language: 'json' } } },
    event: [
      {
        listen: 'test',
        script: {
