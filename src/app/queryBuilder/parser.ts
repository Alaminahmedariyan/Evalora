...
const assertOperatorAllowedForType = (
	field: string,
	operator: string,
	type: FieldType,
) => {
	...
	switch (config) {
		case "number": {
			...
			if (raw.trim() === "" || Number.isNaN(num)) {
				throw new AppError(
					StatusCodes.BAD_REQUEST,
					`Invalid number value for "${field}": "${raw}"`,
				);
			}
			return num;
		}
		case "decimal": {
			try {
				...
				throw new AppError(
					StatusCodes.BAD_REQUEST,
					`Invalid decimal value for "${field}": "${raw}"`,
				);
			}
		}
		case "boolean": {
		...
		case "date": {
			...
			if (Number.isNaN(date.getTime())) {
				throw new AppError(
					StatusCodes.BAD_REQUEST,
					`Invalid date value for "${field}": "${raw}"`,
				);
			}
			return date;
		}
...
const parseSort = (
	...
	if (
		...
		config.sortableFields.includes(query.sortBy)
	) {
...
const parseCsvWhitelisted = (
	...
	const filtered = allowed
		? requested.filter((f) => allowed.includes(f))
		: requested;
