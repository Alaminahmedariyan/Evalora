...
const buildSearchWhere = (
	...
	return {
		OR: searchableFields.map((field) =>
			buildNestedField(field, { contains: search, mode: "insensitive" }),
		),
	};
};
...
/**
 * Resolution order:
 * 1. Caller requested specific fields (?fields=a,b,c) -> flat dynamic select.
 * 2. Config declares a defaultSelect (a fixed shape, may include nested
 *    relations) -> use it as-is. select and include are mutually exclusive
 *    in Prisma, so defaultInclude/requested include are ignored once a
 *    defaultSelect is configured.
 * 3. Otherwise fall back to include-based behavior (defaultInclude +
 *    caller-requested ?include=).
 */
const buildSelectOrInclude = (
	parsed: ParsedQuery,
	config: QueryConfig,
): { select?: Record<string, unknown>; include?: Record<string, unknown> } => {
	const dynamicSelect = buildDynamicSelect(parsed.fields);
	if (dynamicSelect) return { select: dynamicSelect };

	if (config.defaultSelect) return { select: config.defaultSelect };

	const include = buildInclude(parsed.include, config.defaultInclude);
...
export const buildPrismaArgs = (
	...
	const { select, include } = buildSelectOrInclude(parsed, config);
