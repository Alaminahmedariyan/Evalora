...
const createAssessmentVersion = async (id: string, companyId: string) => {
  ...
  if (existing.status === "DRAFT") {
    throw new AppError(StatusCodes.CONFLICT, "Only published or closed assessments can be versioned.");
  }
...
const getAssessmentVersions = async (id: string, companyId?: string) => {
  ...
  const versions = await prisma.assessment.findMany({
    where: { companyId: existing.companyId, slug: existing.slug, deletedAt: null },
    select: {
      id: true,
      title: true,
      slug: true,
...
const restoreAssessmentVersion = async (id: string, companyId: string) => {
  ...
  if (target.isLatestVersion && target.status !== "DRAFT") {
    throw new AppError(StatusCodes.CONFLICT, "This is already the latest published version.");
  }

  const restored = await prisma.$transaction(async (tx) => {
    const latest = await tx.assessment.findFirst({
      where: { companyId, slug: target.slug, isLatestVersion: true, deletedAt: null },
      orderBy: { version: "desc" },
      include: { assessmentProblems: true },
    });

    if (!latest) {
      throw new AppError(StatusCodes.NOT_FOUND, "Latest assessment version not found.");
    }
