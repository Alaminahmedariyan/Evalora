...
var getPaymentById = async (id, requester) => {
  ...
  if (!payment) {
    throw new appError_default(StatusCodes7.NOT_FOUND, "Payment not found.");
  }
...
var verifyCaptcha = async (token) => {
  ...
  if (!response.ok) {
    throw new appError_default(
      StatusCodes9.BAD_GATEWAY,
      "Captcha verification service is unavailable."
    );
  }
  const result = await response.json();
  if (!result.success) {
    throw new appError_default(StatusCodes9.BAD_REQUEST, "Captcha verification failed.");
  }
};
...
var requireRole = (...roles) => {
  return catchAsync(
    async (req, _res, next) => {
      if (!req.user) {
        throw new appError_default(StatusCodes10.UNAUTHORIZED, "You are not logged in.");
      }
...
var getDashboardStats2 = catchAsync(async (_req, res) => {
  ...
  res.status(StatusCodes11.OK).json({
    success: true,
    ...
    cloudinary2.uploader.upload_stream(
      ...
      (error, result) => {
        if (error || !result) {
          return reject(
            new appError_default(
              StatusCodes13.INTERNAL_SERVER_ERROR,
...
var verifyCompany = async (id, actorId) => {
  ...
  if (company.isVerified) {
    throw new appError_default(StatusCodes14.CONFLICT, "Company is already verified.");
  }
...
var restoreAssessmentVersion = async (id, companyId) => {
  ...
  if (!target) {
    throw new appError_default(StatusCodes15.NOT_FOUND, "Assessment version not found.");
  }
...
var saveSubmissionSchema = z3.object({
  selectedOptionIds: z3.array(z3.string().min(1)).min(1).max(10).optional(),
...
var validateRequestWithFile = (schema) => {
  return async (req, _res, next) => {
    try {
      ...
      return next(
        new appError_default(StatusCodes22.BAD_REQUEST, "Invalid JSON in 'data' field.")
      );
    }
    const result = await schema.safeParseAsync(req.body ?? {});
    if (!result.success) {
...
var upsertMyProfile = async (userId, payload, file) => {
  ...
  if (!user) {
    throw new appError_default(StatusCodes23.NOT_FOUND, "User not found.");
  }
  let resumeUrl;
  if (file) {
    const uploaded = await uploadFileToCloudinary(
...
var getMyProfile = async (userId) => {
  ...
  if (!profile) {
    throw new appError_default(StatusCodes23.NOT_FOUND, "Candidate profile not found.");
  }
  return profile;
};
...
var upsertMyProfile2 = catchAsync(async (req, res) => {
  ...
  res.status(StatusCodes24.OK).json({
    success: true,
...
// src/app/modules/candidade/candidate.validation.ts
var urlField = (label) => z6.string().trim().url(`Enter a valid ${label} URL, e.g. https://example.com/you.`).optional();
var upsertProfileSchema = z6.object({
  headline: z6.string().trim().min(2, "Headline must be at least 2 characters.").max(150, "Headline must be at most 150 characters.").optional(),
  bio: z6.string().trim().max(2e3, "Bio must be at most 2000 characters.").optional(),
  phone: phoneSchema,
  location: z6.string().trim().max(150, "Location must be at most 150 characters.").optional(),
  ...
  experienceYears: z6.coerce.number().int("Experience years must be a whole number.").min(0, "Experience years cannot be negative.").max(60, "Enter a realistic number of years.").optional()
});
...
var registerCompany2 = catchAsync(async (req, res) => {
  ...
  res.status(StatusCodes25.CREATED).json({
    success: true,
...
var companyValidation = {
  ...
  updateSubscriptionSchema: z7.object({
    plan: z7.enum(["FREE", "PRO", "ENTERPRISE"], {
...
var revokeConsent = async (userId, consentType) => {
  ...
  if (!existing) {
    throw new appError_default(StatusCodes26.NOT_FOUND, "Consent not found.");
  }
  if (!existing.granted) {
    throw new appError_default(
...
var getSubmissionsForAttempt = async (attemptId, requester) => {
  ...
  if (!attempt) {
    throw new appError_default(StatusCodes28.NOT_FOUND, "Attempt not found.");
  }
  assertCanGrade(
    requester.companyId,
...
var getSubmissionById = async (id, requester) => {
  ...
  if (!submission) {
    throw new appError_default(StatusCodes28.NOT_FOUND, "Submission not found.");
  }
  const attempt = await prisma.assessmentAttempt.findUniqueOrThrow({
...
var getPendingEvaluations = async (assessmentId, companyId) => {
  ...
  if (!assessment) {
    throw new appError_default(StatusCodes28.NOT_FOUND, "Assessment not found.");
  }
  return prisma.submission.findMany({
...
var getInvitationById = async (id, requester) => {
  ...
  if (!invitation) {
    throw new appError_default(StatusCodes30.NOT_FOUND, "Invitation not found.");
  }
...
var acceptInvitation = async (id, userId, email) => {
  ...
  if (invitation.status === "PENDING" && invitation.expiresAt && invitation.expiresAt < /* @__PURE__ */ new Date()) {
    ...
    throw new appError_default(StatusCodes30.GONE, "This invitation has expired.");
  }
...
var inviteCandidates2 = catchAsync(async (req, res) => {
  ...
  res.status(StatusCodes31.CREATED).json({
    success: true,
...
var markAsRead = async (id, userId) => {
  ...
  if (!notification) {
    throw new appError_default(StatusCodes32.NOT_FOUND, "Notification not found.");
  }
...
var getProblemById = async (id, companyId) => {
  ...
  if (!problem) {
    throw new appError_default(StatusCodes35.NOT_FOUND, "Problem not found.");
  }
  return problem;
};
...
var getResultByAttemptId = async (attemptId, requester) => {
  ...
  if (!attempt) {
    throw new appError_default(StatusCodes37.NOT_FOUND, "Attempt not found.");
  }
  const isOwner = attempt.candidateId === requester.id;
  const isOwningRecruiter = requester.companyId !== void 0 && attempt.assessment.companyId === requester.companyId;
  if (requester.role !== "ADMIN" && !isOwner && !isOwningRecruiter) {
    throw new appError_default(
      StatusCodes37.FORBIDDEN,
...
var getResultsForAssessment = async (assessmentId, companyId) => {
  ...
  if (!assessment) {
    throw new appError_default(StatusCodes37.NOT_FOUND, "Assessment not found.");
  }
...
var getUserById = async (id) => {
  ...
  if (!user) {
    throw new appError_default(StatusCodes39.NOT_FOUND, "User not found.");
  }
  return user;
};
